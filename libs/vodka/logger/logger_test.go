...
func TestLogger(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	c = e.NewContext(req, rec, func(c *vodka.Context) error {
		return c.String("test", vodka.StatusTemporaryRedirect)
	})
	h = Logger()
	h(c)
	...
	rec = httptest.NewRecorder()
	c = e.NewContext(req, rec, func(c *vodka.Context) error {
		return c.String("test", vodka.StatusNotFound)
	})
	h = Logger()
	h(c)
...
func TestLoggerTemplate(t *testing.T) {
	...
	e.Get("/", func(c *vodka.Context) error {
		return c.String("Header Logged", vodka.StatusOK)
	})

	req := httptest.NewRequest(vodka.GET, "/?username=apagano-param&password=secret", nil)
	req.RequestURI = "/"
	req.Header.Add(vodka.HeaderXRealIP, "127.0.0.1")
	req.Header.Add("Referer", "google.com")
	req.Header.Add("User-Agent", "dsc/libs/vodka-tests-agent")
