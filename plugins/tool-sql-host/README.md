...
- 每個 `.dsp` 一個獨立 LUA VM；插件內腳本經 `dsc.register_tool` 註冊的工具以
  **插件名為前綴**暴露給宿主：插件 `hello` 註冊的 `greet` → 模型看到 `hello_greet`
  （合成規則見 SDK 的 `dsc.QualifyToolName`）。前綴表達的是**來源**而非**能力**：這些
  工具可以是任何事（打招呼、寫筆記…），故不按載體技術取名（`sql_` 會讓模型誤以為與
  SQL 查詢有關）；而固定用載體代號（如 `dsp_`）也只説得出「某個 `.dsp` 插件」、
  説不出是哪個——插件一多，前綴就等於沒説。用插件名後，工具名與 `list_dsp_plugins`
  列出的插件名**逐字對應**，歸屬一眼可查。腳本內引用**自身**工具名時用 `dsc.plugin.name`
  拼前綴（示例見 `examples/hello`），別把帶前綴的名字寫死。庫內保留表仍用 `dsp_` 前綴
  （`dsp_meta`/`dsp_blobs`/`dsp_state`/`dsp_log`）——那是同一個庫文件內的命名空間，與
  下發給模型的工具名無關；腳本側的 `dsc.sql.*` 內建也仍叫 `sql`，它們命名的是真實
  SQL 操作，名副其實。
...
4. `dsp_meta` 表必須存在且含 `name`（插件名）與 `language`（載體語言）。插件名僅允許
   `[A-Za-z0-9_-]`（它同時是該插件註冊工具的名前綴，見上「目錄約定與加載」），故取名
   應短而可辨識。
