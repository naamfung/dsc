...
- **插件安裝/管理（模型可自助）**：宿主內置 `install_dsc_plugin` / `uninstall_dsc_plugin` / `list_dsc_plugins` 三個模型工具（指 DSC 自身的二进制插件——本机 Go 程序、經 dsc-sdk 構建、go-plugin/gRPC 加載，有別於 Go 標準庫的 plugin 包；對齊 SKILL 安裝，經聚合 Tool 服務暴露給模型）。嚴格命約定（插件目錄 `plugins/<type>-<name>/`、執行檔 `<type>-<name><ext>`、`type`∈tool/llm/agent/policy/dsc、`name` 僅 `[A-Za-z0-9_-]`），寫 config 前先備份（`config.yaml.<ts>.bak`），並「干跑=live 加載」驗證類型/元數據一致才落盤、失敗回滾（刪已拷目錄、config 未變），防止模型寫壞配置導致起不來。新裝插件即時熱加載、無需重啟；模型據「能否加載 ACTIVE」判斷安裝是否正確，可再用 `uninstall_dsc_plugin` 清理。
