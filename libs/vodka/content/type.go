...
// Package content provides content negotiation handlers for the vodka.
package content
...
// MIME types
const (
	JSON = vodka.MIME_JSON
	XML  = vodka.MIME_XML
	XML2 = vodka.MIME_XML2
	HTML = vodka.MIME_HTML
)
...
var DataWriters = map[string]vodka.DataWriter{
	JSON: &JSONDataWriter{},
	XML:  &XMLDataWriter{},
	XML2: &XMLDataWriter{},
	HTML: &HTMLDataWriter{},
...
//
// The negotiator will set the "Content-Type" response header as the chosen MIME type. It will call vodka.Context.SetDataWriter()
...
func TypeNegotiator(formats ...string) vodka.Handler {
	if len(formats) == 0 {
		formats = []string{HTML}
	}
...
// HTMLDataWriter sets the "Content-Type" response header as "text/html; charset=UTF-8" and calls vodka.DefaultDataWriter to write the given data to the response.
...
func (w *HTMLDataWriter) Write(res http.ResponseWriter, data interface{}) error {
	return vodka.DefaultDataWriter.Write(res, data)
}
