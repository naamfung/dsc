...
func TestCookieEncodeDecode(t *testing.T) {
	...
	var dst = make(map[interface{}]interface{})
...
func TestParseConfig(t *testing.T) {
	s := `{"cookieName":"iVodkaSessionId","gcLifetime":3600}`
	...
	if cf.CookieName != "iVodkaSessionId" {
	...
	cc := `{"cookieName":"iVodkaSessionId","enableSetCookie":false,"gcLifetime":3600,"providerConfig":"{\"cookieName\":\"iVodkaSessionId\",\"securityKey\":\"iVodkaCookiehashkey\"}"}`
	...
	if cf2.CookieName != "iVodkaSessionId" {
		t.Fatal("parseconfig get cookiename error")
	}
	...
	if cconfig.CookieName != "iVodkaSessionId" {
		t.Fatal("providerConfig get cookieName error")
	}
	if cconfig.SecurityKey != "iVodkaCookiehashkey" {
