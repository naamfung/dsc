...
- **上下文压缩（compaction-basic，插件化）**：宿主压缩引擎整体迁入 `dsc-system` 驻留（对齐 DSH compaction-basic，协议与 agent 零改动）：`agent/pre-step` 槽压力驱动——估算用量超阈值（默认窗口 80%，`DSC_COMPACTION_THRESHOLD` 可调，对齐 compaction-basic；agent 经 `[DSC_COMPACTION_BACKEND_ACTIVE]` 后端标记检测自动跳过内联压缩，接管靠能力声明驱动、与阈值无关）时保留尾部（16%、≥1024 token）并把未压缩前段经聚合 LLM 生成摘要（interconnect 互通；未互联或调用失败退化截断式），以 `{"messages": [...]}` 改写本步请求；LLM 请求溢出（`agent/request-error` 的 context_window_exceeded）时截断式紧急压缩并返回 `{"retry": true}` 让宿主重试；逐 step 复用靠 per-session 状态指纹（历史被改写自动作废重估），`DSC_COMPACTION_BASIC_CONTEXT_WINDOW`（默认 131072，0 禁用）与 `DSC_COMPACTION_BASIC_DIR` 可调；config.yaml `compaction-basic: dsc-system`（或 `dsc-billion-context`）显式选后端，宿主仅验证 `Provides compaction-basic` 能力声明。
