...
// v3 引入 system/message surface 事件作为 node 0（对齐 DSH v3 system/message
// surface node 0 设计）：system prompt 由事件日志派生，不再由调用方参数前置。
// v2 日志无 system/message 事件——首条 surface 节点通常是 user/message。
// 迁移在事件序列首部追加一条空 content 的 system/message 事件（surface append），
// 让 v3 surface 不变量（node 0 = system/message）成立；空 content 占位投影为 nil
// （不派生模型消息），保持 v2 会话恢复后派生消息序列与原文等价。
// v3 同时保留 v2 已有的 llm/attempt 诊断事件类型（log-only，不影响 surface）。
func migrateV2ToV3(events []*Event) ([]*Event, error) {
        if len(events) == 0 {
                return events, nil
        }
        // 检查首条事件是否已是 system/message（避免重复迁移）
        if events[0].Type == SystemMessage {
                return events, nil
        }
        // 在首部插入空 system/message 占位节点（surface append，时间戳取首条事件时间，
        // 避免相对时间倒序）。空 content 投影为 nil，不改变模型派生消息序列。
        placeholder := &Event{
                Seq:     0, // 重排后由 SaveWithFormat 写实际序号；此处仅占位
                Time:    events[0].Time,
                Type:    SystemMessage,
                Data:    &SystemMessageData{Turn: 0, Step: 0, Content: ""},
                Surface: &SurfaceOp{Op: SurfaceAppend},
        }
        // 后续事件 Seq 整体后移 1；Surface replace 的 Start/End 引用也需同步 +1
        out := make([]*Event, 0, len(events)+1)
        out = append(out, placeholder)
        for _, ev := range events {
                ev.Seq++
                if ev.Surface != nil {
                        if ev.Surface.Op == SurfaceReplace {
                                ev.Surface.Start++
                                ev.Surface.End++
                        }
                }
