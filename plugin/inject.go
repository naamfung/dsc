...
// pluginLoadedLocked 判断插件当前是否已加载到 Manager（agent 或 provider）。
func (m *Manager) pluginLoadedLocked(name string) bool {
	if _, ok := m.agents[name]; ok {
		return true
	}
...
func (m *Manager) deferPendingLocked(entry PluginEntry) error {
	...
	m.logger.Info("plugin deferred to pending (dependency not satisfied)", "name", entry.Name)
	return nil
}
...
func (m *Manager) injectionEntryLocked(entry PluginEntry) error {
	...
	case "llm", "tool", "policy":
		if m.broker == nil {
			return fmt.Errorf("broker not available, cannot inject plugin type %s", entry.Type)
		}
		if !m.entryDepsSatisfiedLocked(entry) {
	...
	default:
		return fmt.Errorf("unsupported plugin type for injection: %s", entry.Type)
	}
...
func (m *Manager) repairPendingLocked() error {
	for {
		...
		for name, entry := range m.pendingEntries {
			...
			case "tool", "policy":
				if m.broker == nil {
					return fmt.Errorf("broker not available, cannot promote plugin %s", name)
				}
