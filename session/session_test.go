...
// appendSystemPrompt 在 surface node 0 追加一条 system/message（v3 不变量：
// 会话首条 surface 节点必须是 system/message）。测试 helper——生产代码经
// agent-react-loop 的 commitSystemPrompt 走 ProjectSystemPrompt 决策路径。
func appendSystemPrompt(t *testing.T, s *Session, content string) *Event {
	t.Helper()
	return s.Append(SystemMessage, &SystemMessageData{Content: content}, &SurfaceOp{Op: SurfaceAppend})
}
...
func TestSurfaceReplaceShadowsRange(t *testing.T) {
	...
	// 压缩：摘要 replace 遮蔽 seq 2..3（u2、u3）——system 是 seq 0，不受影响
	summary := s.Append(CompactionSummary, &CompactionSummaryData{Content: "sum"}, &SurfaceOp{
		Op: SurfaceReplace, Start: 2, End: 3,
	})
	...
	// 日志本体无损：所有事件仍在
	if s.Len() != 6 {
		t.Fatalf("log len = %d, want 6 (events preserved)", s.Len())
	}
	// replace 遮蔽的节点正是声明范围
	nodes := s.SurfaceNodes()
	wantNodes := []int{0, 1, summary.Seq, 4}
