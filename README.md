...
| 上下文壓縮          | `compaction-basic`：thresholdRatio / retainRatio                                  | `dsc-system` 驻留压缩（自宿主迁入，宿主零压缩代码）：45% 阈值接管（agent 内联 80% 兜底）、16% 尾部保留（≥1024 token）、聚合 LLM 摘要（未互联退化截断式）、per-session 状态指纹复用、溢出紧急压缩重试                                                                                                                                                                              |
