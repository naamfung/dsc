...
func Serve(opts *ServeConfig) {
	...
	// 信号处理（对齐 AGENTS.md 第7条宿主侧 graceful_shutdown 范式）：
	//   - SIGINT (Ctrl+C)：原 hashicorp 行为是「吃掉、忽略」——保留，避免用户误触
	//     Ctrl+C 杀掉插件进程（宿主管理生命周期，应由宿主发 Shutdown RPC）。
	//   - SIGTERM / SIGHUP / SIGQUIT：触发 GracefulStop（让 in-flight RPC 自然结束），
	//     这是宿主侧 client.Kill 在 SIGKILL 兜底前给插件的优雅退出窗口。
	//
	// Windows：os/signal 不捕获 SIGTERM（Windows 无此信号概念），改由宿主直接发
	// Shutdown RPC（见 client.Close）；SIGINT 由 SetConsoleCtrlHandler 等价处理。
	if opts.Test == nil {
		ch := make(chan os.Signal, 4)
		signal.Notify(ch, os.Interrupt)
		// Unix 专属信号：syscall.SIGTERM / SIGHUP / SIGQUIT
		// 在 Windows 上 syscall.SIGTERM 等仍可注册但不会被内核发——
		// 故在不影响 Windows 行为的前提下增强 Unix 优雅关闭。
		signal.Notify(ch, unixSigterm, unixSighup, unixSigquit)
		go func() {
			for sig := range ch {
				if sig == os.Interrupt {
					// 保留原行为：吃掉 SIGINT，宿主主动管理生命周期
					logger.Trace("plugin received SIGINT, ignoring (host manages lifecycle)")
					continue
				}
				// SIGTERM / SIGHUP / SIGQUIT：触发 GracefulStop
				logger.Info("plugin received signal, graceful stopping", "signal", sig.String())
				if s, ok := server.(*GRPCServer); ok {
					go s.GracefulStop() // 异步避免阻塞信号 handler
				} else {
					_ = listener.Close()
				}
				return
