...
func TestContent(t *testing.T) {
	...
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusMethodNotAllowed, err.(*vodka.HTTPError).StatusCode())
	}
	...
	err = h(c)
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusNotFound, err.(*vodka.HTTPError).StatusCode())
	}
...
func TestServer(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.status == 0 {
			...
			if assert.NotNil(t, err, test.id) {
				assert.Equal(t, test.status, err.(*vodka.HTTPError).StatusCode(), test.id)
			}
		}
	}
	...
	h2 := Server(PathMap{"/css": "/testdata/css"}, ServerOptions{
		...
		Allow: func(c *vodka.Context, path string) bool {
