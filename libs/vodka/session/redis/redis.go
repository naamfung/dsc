...
func (rs *SessionStore) Release(ctx *vodka.Context) (err error) {
	var b []byte
...
func (rp *Provider) Read(sid string) (vodka.RawStore, error) {
	c := rp.poollist.Get()
	defer c.Close()
...
func (rp *Provider) Regenerate(oldsid, sid string) (vodka.RawStore, error) {
	c := rp.poollist.Get()
	defer c.Close()
