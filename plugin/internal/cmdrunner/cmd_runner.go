...
// GracefulKill 实现 runner.GracefulRunner 接口：发 SIGTERM 让插件进程开始优雅退出。
//
// 调用方（client.Kill）会先调 GracefulKill（SIGTERM），等待 GracefulShutdownTimeout
// 让进程跑完 defer / 关闭 in-flight RPC，超时后才调 Kill（SIGKILL）兜底。
// Windows 上 sendTermination 返回 nil（无 SIGTERM 概念），此处等价于 no-op。
func (c *CmdRunner) GracefulKill() error {
	if c.cmd.Process == nil {
		return nil
	}
	return sendTermination(c.cmd.Process)
}
