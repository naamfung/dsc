...
//
// 优雅关闭：原实现把 on-stop 钩子放在 defer 里，但末尾的 os.Exit(1) 会跳过所有
// defer，导致 OnStop 在生产中从未执行（死代码）。这里把 os.Exit(1) 替换为
// return——plugin.Serve 在收到宿主 Shutdown RPC 后正常返回（不再视为错误），
// return 让 defer 正常执行 OnStop，给插件一个清理资源（关闭 DB / HTTP 服务 /
// flush 日志）的机会。真正的异常启动错误（如 validate 失败）仍走 os.Exit(2)。
func (s *SDK) Serve() {
	...
	// plugin.Serve 返回有两种情形：
	//   1. 宿主发了 Shutdown RPC（grpc_controller.Shutdown 异步触发 GracefulStop
	//      后 gRPC 服务器自然结束）—— 这是正常退出，让 OnStop defer 跑完。
	//   2. 启动失败 / 宿主异常断开 —— 异常退出，OnStop 也应尝试清理（防资源泄漏）。
	// 两种情形都让 return 走完 defer，再以 exit code 1 兜底退出。
	fmt.Fprintln(os.Stderr, "dsc-sdk: plugin serve exited")
}
