...
func applyStandardCompression(r PruneRange, messages []CoreMessage, state *CompressionState, result *CompressionResult, runID string, covered map[string]bool) {
        ...
        for i := startIdx; i <= endIdx; i++ {
                ...
                // summary 均为 system role）——defense-in-depth：即使模型在 compress 调用中
                // 指定了包含 system 的范围，也不将其压缩进新块。对齐 DSH "node 0 is never
                // inside a compaction range" 与 agent-react-loop compaction 范围 [nodes[1], ...]
                // 跳过 system node 的同款语义。
