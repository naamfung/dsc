...
func ComputeCompressibleRanges(messages []CoreMessage, state *CompressionState, config Config) []CompressibleRange {
	...
	for i, msg := range messages {
		...
		// 跳过块 summary（已渲染的 summary 消息，ID 形如 acp_summary_bN）
		// 这些消息本身就是上一轮压缩的产物，不应再被纳入可压缩范围——
		// 否则 nudge 会建议模型"压缩 summary"，模型调用 compress 时 applyStandardCompression
		// 又因 isRenderedSummary 跳过（prune.go:83），最终报 "no compressible messages"，
