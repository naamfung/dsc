...
// Package auth provides a set of user authentication handlers for the vodka.
package auth
...
type BasicAuthFunc func(c *vodka.Context, username, password string) (Identity, error)
...
func Basic(fn BasicAuthFunc, realm ...string) vodka.Handler {
	name := DefaultRealm
	if len(realm) > 0 {
		name = realm[0]
	}
	return func(c *vodka.Context) error {
		...
		return vodka.NewHTTPError(http.StatusUnauthorized, e.Error())
	}
}
...
type TokenAuthFunc func(c *vodka.Context, token string) (Identity, error)
...
// When authentication fails, a "WWW-Authenticate" header will be sent, and an http.StatusUnauthorized
// error will be returned.
func Bearer(fn TokenAuthFunc, realm ...string) vodka.Handler {
	name := DefaultRealm
	if len(realm) > 0 {
		name = realm[0]
	}
...
func Query(fn TokenAuthFunc, tokenName ...string) vodka.Handler {
	...
	return func(c *vodka.Context) error {
		...
		if err != nil {
			return vodka.NewHTTPError(http.StatusUnauthorized, err.Error())
		}
		c.Set(User, identity)
		return nil
	}
...
type JWTOptions struct {
	...
	// a function that handles the parsed JWT token. Defaults to DefaultJWTTokenHandler, which stores the token in the vodka.context with the key "JWT".
	TokenHandler JWTTokenHandler
	// a function to get a dynamic VerificationKey
	GetVerificationKey VerificationKeyHandler
}

// DefaultJWTTokenHandler stores the parsed JWT token in the vodka.context with the key named "JWT".
func DefaultJWTTokenHandler(c *vodka.Context, token *jwt.Token) error {
	c.Set("JWT", token)
	return nil
}
...
// will be stored in the vodka.context with the key named "JWT". Other handlers can retrieve this token to obtain
...
func JWT(verificationKey string, options ...JWTOptions) vodka.Handler {
	...
	return func(c *vodka.Context) error {
		...
		if message != "" {
			return vodka.NewHTTPError(http.StatusUnauthorized, message)
		}
