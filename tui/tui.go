...
type Model struct {
        ...
        // 复制暂时失效。
        //
        // 注意：此前模型工作期间（thinking/streaming）也会自动释放给终端以便原生拖选复制，
        // 但这会让滚轮事件被终端吃掉——鼠标滚到输入框上时滚的是 textarea 的历史缓冲，
        // 滚到正文区也无法翻阅历史。现统一保持 CellMotion 捕获：滚动条拖拽、滚轮翻阅
        // 在模型工作期间同样可用；正文拖选复制在 streaming 期间由 MouseClickMsg 处理
        // 路径单独禁用（避免行号变化导致选区漂移），与鼠标捕获开关解耦。
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
        ...
        case questionMsg:
                ...
                // 同步 viewport 实际高度：vpHeight() 已扣减覆盖层行数，
                // 但 m.viewport 内部缓存的 Height 仍是覆盖层激活前的旧值——
                // 不调用 SetHeight 同步的话，viewport.View() 会输出多余行数，
                // 把覆盖层推到终端可视区之外（AltScreen 截断），表现为模型
                // 提问后选项被切掉一部分。对齐 WindowSizeMsg 的处理路径。
                m.viewport.SetHeight(m.vpHeight())
                m.render()
                return m, nil
...
// scopeLabel 显示当前工作范围（左下角状态栏）：
// 始终展示当前工作区的真实目录基础名（限长，避免超长真实目录破坏布局），
// 沙箱模式经后缀标记呈现（如「桌面 🔒」「桌面 ✎」「桌面 ⚡」），
...
func (m *Model) scopeLabel() string {
        ...
        // 沙箱模式后缀标记（与目录名同栏展示，不替换目录名）
        if m.manager != nil {
                switch m.manager.GetSandboxPolicy() {
                case core.SandboxReadOnly:
                        name += " 🔒" // read-only
                case core.SandboxWorkspaceWrite:
...
// 鼠标策略：始终应用内捕获（CellMotion），滚轮滚动、滚动条拖拽、正文拖拽选中
// 自动复制均内建工作；用户可经 DSC_DISABLE_MOUSE=1 永久释放给终端（恢复终端
// 原生文字选中/复制，代价是应用内滚轮与拖选失效）。
//
// 模型工作期间（thinking/streaming）此前会切到 MouseModeNone 把鼠标释放给终端，
// 但这导致滚轮事件被终端吃掉（鼠标在输入框上时滚的是 textarea 历史缓冲、在正文
// 区也无法翻阅历史）——与「滚动条拖拽在模型工作期间同样可用」的既有设计冲突。
// 现统一保持 CellMotion：模型工作期间滚轮、滚动条拖拽均正常；正文拖选复制在
// streaming 期间由 MouseClickMsg 处理路径单独禁用（避免行号变化导致选区漂移）。
//
