...
var defaultOtions = Options{"memory", `{"cookieName":"VodkaSessionID","gcLifetime":3600}`}

//var defaultOtions = Options{"file", `{"cookieName":"iVodkaSessionId","gcLifetime":3600,"providerConfig":"./data/session"}`}

//var defaultOtions = Options{"redis", `{"cookieName":"iVodkaSessionId","gcLifetime":3600,"providerConfig":"127.0.0.1:6379"}`}
...
type Store interface {
	...
	Read(string) (vodka.RawStore, error)
	// Destory deletes a session.
	Destory(*vodka.Context) error
	// RegenerateId regenerates a session store from old session ID to new one.
	RegenerateId(*vodka.Context) (vodka.RawStore, error)
...
func setup(op ...Options) error {
	...
	log.Println("[VODKA] Session Config:", option)
...
func Sessioner(op ...Options) vodka.Handler {
	...
	return func(c *vodka.Context) error {
		...
		if flashIf != nil {
			...
			if flasho, okay := flashIf.(*vodka.Flash); okay {
				if flashVals, _ = url.ParseQuery(flasho.Encode()); len(flashVals) > 0 {
					flash := vodka.Flash{Values: url.Values{}}
		...
		if !has {
			c.Flash = NewFlash(new(vodka.Context))
...
func GetFlash(c *vodka.Context) *vodka.Flash {
	if store := GetStore(c); store != nil {
		if tmp := store.Get(SESSION_FLASH_KEY); tmp != nil {
			if flash, okay := tmp.(*vodka.Flash); okay {
				return flash
			}
		}
	}
...
func FlashValue(c *vodka.Context) vodka.Flash {
	if tmp := c.Get(CONTEXT_FLASH_KEY); tmp != nil {
		return tmp.(vodka.Flash)
	}
	return vodka.Flash{}
}

func NewFlash(ctx *vodka.Context) *vodka.Flash {
	return &vodka.Flash{FlashNow: vodka.FlashNow, Ctx: ctx, Values: url.Values{}, ErrorMsg: "", WarningMsg: "", InfoMsg: "", SuccessMsg: ""}
}
