...
// presetsDir 解析 preset 配置目录路径（与 SwitchMode 同一逻辑）。
// 优先 m.config.ExecDir/config/presets，退回到可执行文件所在目录的
// config/presets，再退回到 ./config/presets。未配置 ExecDir 时尝试
// os.Executable() 推断。
func (m *Manager) presetsDir() string {
	if m.config.ExecDir != "" {
		return PJoin(m.config.ExecDir, "config", "presets")
	}
	if execPath, err := os.Executable(); err == nil {
		return PJoin(PDir(execPath), "config", "presets")
	}
	return "./config/presets"
}

// ListModes 列出当前 presets 目录下可用的模式名（<name>.yaml 去扩展名）。
// 用于 TUI 动态生成 /mode 斜杆命令菜单，避免硬编码固定模式——用户新增
// preset 配置文件（如 thin.yaml）时，斜杆命令自动出现 /mode thin。
// 返回按字母序排序；目录不可读时返回 nil（视为无可用模式，TUI 会回退到内置默认）。
func (m *Manager) ListModes() []string {
	...
	for _, e := range entries {
		...
		mode := strings.TrimSuffix(name, ".yaml")
		if mode == "" {
			continue
		}
