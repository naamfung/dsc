...
//
// 事件经 EventBus 的 OnAny → dispatchEventToPlugins 自动同步分发给所有注册
// PluginHookService.OnEvent 的插件（如 notify），由订阅插件程序性响应
// （播放音效等），不涉及模型。waterfall 模式事件（agent/pre-step 等）由
// agent 直接调 dispatchEventToPlugins 取回 result——不经此 OnAny 路径。
