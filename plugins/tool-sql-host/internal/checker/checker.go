...
func dscType() typ.Type {
	...
	return typ.NewRecord().
		...
		Field("store", typ.NewRecord().
			...
			Build()).
		Field("sql", typ.NewRecord().
			Field("query", typ.Func().Param("stmt", typ.String).OptParam("params", typ.Any).Returns(typ.Any).Build()).
			Field("exec", typ.Func().Param("stmt", typ.String).OptParam("params", typ.Any).Returns(typ.Integer).Build()).
			Field("tables", typ.Func().Returns(typ.Any).Build()).
			Build()).
		Field("dsp", typ.NewRecord().
			Field("require", typ.Func().Param("name", typ.String).Returns(typ.Any).Build()).
			Field("asset", typ.Func().Param("name", typ.String).Returns(typ.Any).Build()).
			Build()).
		Field("plugin", typ.NewRecord().
			Field("name", typ.String).
			Field("path", typ.String).
			Field("readonly", typ.Boolean).
			Build()).
