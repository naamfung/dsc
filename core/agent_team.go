	...
	for i := 1; i < len(out); i++ {
		for j := i; j > 0; j-- {
			if out[j].Priority > out[j-1].Priority ||
				(out[j].Priority == out[j-1].Priority && out[j].CreatedAt < out[j-1].CreatedAt) {
...
// Receive 接收指定代理的消息（to == agentName 或 to == "" 广播）。
func (mq *MessageQueue) Receive(agentName string) []*TeamMessage {
	mq.mu.Lock()
	defer mq.mu.Unlock()
	var out []*TeamMessage
	for i := len(mq.messages) - 1; i >= 0; i-- {
		m := mq.messages[i]
...
// StringBuilder 简易字符串构建器（避免引入 strings.Builder 的重复）。
...
// unused but referenced by interface
