...
func main() {
	sdk := dsc.New(dsc.Config{
		...
		// 宿主在 -input 单发回合结束前据此判定是否宽限音效播完
		// （mgr.HasPluginProvidingCapability("notify")，见 main.go），而非按名探测
		// dsc-notify（避免绑死具体插件名）。
