...
func TestRunLoopLimitedHistoryInjection(t *testing.T) {
	...
	// v3 不变量：surface node 0 必须是 system/message（runLoop 每步经
	// commitSystemPrompt 检测变更并提交到事件日志）。测试预置 system 节点
	// 模拟生产 RunStream 启动时已提交的 system prompt。
	a.sess.Append(session.SystemMessage, &session.SystemMessageData{Content: "sys"},
	...
	res, err := a.runLoop(context.Background(), "继续", nil, func(*core.RunStreamResponse) {})
	if err != nil {
	...
	// [q3, a3] + 当前输入「继续」= 3 条 + system（surface node 0）= 4 条
	t.Logf("historyInjection=%d streamCalls=%d streamMsgs=%d", a.historyInjection, llm.streamCalls, len(msgs))
	if len(msgs) != 4 {
		t.Fatalf("受限后主请求消息数 = %d, 期望 4（system + 最近 2 条历史 + 当前输入）", len(msgs))
	}
	// msgs[0] 为 system（surface node 0 派生）
	if msgs[0].Role != "system" {
		t.Fatalf("首条应为 system, 实际 (%s, %q)", msgs[0].Role, msgs[0].Content)
	}
	// msgs[1] 为 q3（最近 2 条历史回退到 user 边界）
	if msgs[1].Role != "user" || msgs[1].Content != "q3" {
		t.Fatalf("第二条应为 user(q3), 实际 (%s, %q)", msgs[1].Role, msgs[1].Content)
	}
